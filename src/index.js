...
app.get("/home/:rollNumber", async (req, res) => {
  try {
    ...
    const studentData = await collection.findOne({
      username: req.params.rollNumber,
    })

    const attendanceRecords = await Attendance.find({
      "students.rollNumber": req.params.rollNumber,
    })
...
app.get("/lecturerdash/:username", async (req, res) => {
  try {
    const lecturerData = await collection2.findOne({
    ...
    const studentsData = await collection.find({
      class: { $in: lecturerData.classes },
    })
...
app.post("/api/submitAttendance", async (req, res) => {
  try {
    ...
    for (const student of students) {
      ...
      const dateIndex = studentDoc.dayWiseAttendance.findIndex(
        (entry) => entry.date === date
      )

      if (dateIndex !== -1) {
        // If the date exists, update the numberOfClasses
        studentDoc.dayWiseAttendance[dateIndex].numberOfClasses += isPresent
          ? 1
          : 0
      } else {
